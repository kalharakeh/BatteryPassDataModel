...
- Preserve the existing admin creation checks and interlocks by moving shared creation behavior into a common service used by both the admin UI and external API.
...
The API endpoint must not bypass or weaken the existing admin flow protections. Implementation should extract a shared battery creation service that both `POST /admin/batteries/create` and `POST /api/external/v1/batteries` call. The controllers should only translate transport-specific input and output; the shared service owns the invariants.

The shared service must preserve these interlocks:

- Battery ID is generated by `BatteryIdService`, never supplied by the caller.
- Passport ID is not supplied or created during battery creation.
- Serial number uniqueness is checked globally before insert and protected again by MongoDB duplicate-key handling.
- Battery Family, Battery Model, and software version are resolved from the saved product template catalog.
- Software version must belong to the selected Battery Family and Battery Model.
- Cluster is required and must exist.
- External API calls must pass token cluster-scope authorization before creation.
- Creation writes attribution fields and a `battery.created` audit event.
- Invalid requests fail before any battery document is inserted.

## Creation Flow

The endpoint uses the same shared creation service as the admin flow:
...
- Admin and API creation paths use the same shared creation service or equivalent tested shared validation boundary.
